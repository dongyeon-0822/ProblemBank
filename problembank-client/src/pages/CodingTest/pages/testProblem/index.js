...
function TotalProblemsPage(props) {
    ...
    React.useEffect(() => {
        dispatch(getProblemData()).then(response => {
            ...
            var sliceProblems_2 = [];
            var sliceProblems_3 = [];
            sliceProblems_1= (data.filter(element => element.level === '하')); 
            sliceProblems_2= (data.filter(element => element.level === '중')); 
            sliceProblems_3= (data.filter(element => element.level === '상')); 
            ...
            for(var i=0;i<2;i++){
                var p = randomitem(sliceProblems_1);
                sliceProblems.push(p);
            }
            for(var i=0;i<2;i++){
                var p = randomitem(sliceProblems_2);
                sliceProblems.push(p);
            }
            for(var i=0;i<1;i++){
                var p = randomitem(sliceProblems_3);
                sliceProblems.push(p);
            }
    ...
    const handleChangeDisplayPro = (e) => {
        ...
        setCountDisplayProblem(5);

        const sliceProblems = problems.slice(0, Number(5))
        setResultProblem(sliceProblems);
